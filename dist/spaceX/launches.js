...
class Launches {
    ...
    getLaunchesByRange(start, end) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                // Get the date as a moment object
                const dateAsMomentStart = moment_1.default(start, 'YYYY');
                const dateAsMomentEnd = moment_1.default(end, 'YYYY');
                // Check that the date string is valid, and that the date is before or equal to today's date
                // moment() creates a moment with today's date
                if (!dateAsMomentStart.isValid() || !dateAsMomentStart.isSameOrBefore(moment_1.default())) {
                    return [{ error: 'invalid start year' }];
                }
                if (!dateAsMomentEnd.isValid() || !dateAsMomentEnd.isSameOrBefore(moment_1.default())) {
                ...
                while (dateAsMomentStart <= dateAsMomentEnd) {
                    const parsed = yield this.requestLaunchesByYear(dateAsMomentStart.format('YYYY'));
